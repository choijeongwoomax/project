...
dest_points = [
    (img_width - 100, img_height),  # 좌측 하단 -> 우측 하단
    (img_width - 220, 0),  # 좌측 상단 -> 우측 상단
    (img_width - 420, 0),  # 우측 상단 -> 좌측 상단
    (img_width - 635, img_height)   # 우측 하단 -> 좌측 하단
]
...
def process_frame(capture, callback, output_path):
    ...
    fourcc = cv2.VideoWriter_fourcc(*'XVID')  # XVID 코덱 설정
...
if __name__ == '__main__':
    ...
    output_path = 'output_video.avi'  # XVID로 저장할 경우 AVI로 설정

...
class Curves:
  ...
  def next_midx(self, current, pixel_indices):
    if len(pixel_indices) > self.min_pix:
      current = int(np.mean(self.all_pixels_x[pixel_indices]))
    return current
  ...
  def get_real_curvature(self, xs, ys):
    ...
    if len(xs) == 0 or len(ys) == 0:
        raise ValueError("xs or ys is empty")
  ...
  def fit(self, binary):
    ...
    if len(self.left_pixels_x) == 0 or len(self.left_pixels_y) == 0:
      raise ValueError("No valid left lane pixels found.")
    if len(self.right_pixels_x) == 0 or len(self.right_pixels_y) == 0:
      raise ValueError("No valid right lane pixels found.")
